...
- recent files — _возвращены [ADR-0009](adr_0009_shell_redesign.md), Rule 8: «Недавние» на стартовом экране, с ограничениями_
...
> **Уточнено [ADR-0009](adr_0009_shell_redesign.md), Rules 3 и 8:** на стартовом экране кнопка называется «Новый документ» (`⌘N`) и стоит третьей после «Открыть файл…» и «Открыть папку…»; в строке окна тот же сценарий запускает «+» после вкладок. Семантика черновика выше не меняется.
...
> **Частично заменено [ADR-0009](adr_0009_shell_redesign.md), Rule 4.** Меню открывается кнопкой ⋯ справа в строке окна; первый уровень — команды с сочетаниями и «Настройки…», «Close file» заменён на «Закрыть вкладку». Принцип transient overlay вместо постоянной панели и поведение команд открытия и закрытия сохраняются. Текст ниже сохранён как история решения.
...
> **Частично заменено [ADR-0009](adr_0009_shell_redesign.md), Rules 5 и 7.** Разделение чтения и приложения сохраняется, но: тема переехала в карточку Aa к параметрам чтения; настройки приложения — одна модальная карточка «Настройки» внутри окна (`⌘,`, меню ⋯, ссылка из Aa) вместо цепочки экранов в popover; «About» — её нижняя строка.
