...
- кисть `MmDiagramBackgroundBrush` в `Themes/Colors.axaml`: в светлой теме `Transparent` (диаграмма прямо на странице, как раньше), в тёмной — `#EAE6E0`, oklch(0.926 0.009 78), тёплый светлый тон палитры; контраст со штрихами Mermaid `#333` — не ниже 7:1;
...
- поле вокруг картинки 1em и скругление как у блока кода — одинаковые в обеих темах, чтобы смена темы не сдвигала документ;
